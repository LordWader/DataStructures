...
public class Main {
    ...
    static class NaiveSegmentedTree {
        ...
        private long firstAbove(long value, int x, int lx, int rx) {
            if (rx - lx == 1) {
            ...
            int mid = (lx + rx) / 2;
            if (tree[2 * x + 1].value >= value) {
                return firstAbove(value, 2 * x + 1, lx, mid);
            } else {
                return firstAbove(value, 2 * x + 2, mid, rx);
            }
 
        }
 
        public long firstAbove(long k) {
            return firstAbove(k, 0, 0, this.size);
        }
